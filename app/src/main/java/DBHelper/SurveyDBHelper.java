...
import PassiveDataHelper.Call_Log;
...
public class SurveyDBHelper extends SQLiteOpenHelper {
    ...
    private static final String SQL_CREATE_CALLLOG_MOODCIRCLE =
            "CREATE TABLE CALLLOG_MOODCIRCLE (phonenumber varchar(30), date DATE, duration varchar(20))";
    private static final String SQL_CREATE_SMSLOG_MOODCIRCLE =
            "CREATE TABLE SMSLOG_MOODCIRCLE (date varchar(50), phonenumber varchar(50), message varchar(255))";
...
//        throw new UnsupportedOperationException("Not yet implemented");
     ...
     * @return a Cursor containing the data
     */
    public static Cursor getFirst60Entries(Context context, String tableName) {

        SQLiteDatabase db = getInstance(context).getReadableDatabase();
        Cursor cursor = db.rawQuery("select * from " + tableName + " ORDER BY " + EnergyEntry.COLUMN_NAME_TIME + " ASC LIMIT 60", null);
        // db.close();
        return cursor;
//   throw new UnsupportedOperationException("Not yet implemented");
    }
     ...
     */
    public static void deleteNEntries(Context context, int n, String tableName) {

        SQLiteDatabase db = getInstance(context).getWritableDatabase();
        Cursor cursor = db.rawQuery("select " + EnergyEntry.COLUMN_NAME_TIME + " from " + tableName + " ORDER BY " + EnergyEntry.COLUMN_NAME_TIME + " ASC LIMIT " + n, null);
        cursor.moveToFirst();
        int dateIndex = cursor.getColumnIndex(SurveyDBHelper.EnergyEntry.COLUMN_NAME_TIME);
        Log.v(TAG, cursor.getString(dateIndex));
        String sql = "delete from " + tableName + " where " + EnergyEntry.COLUMN_NAME_TIME + " = " + "\'" + cursor.getString(dateIndex) + "\'";
        Log.v(TAG, sql);
        db.execSQL(sql);
